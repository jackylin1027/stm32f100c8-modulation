...
#define MODBUS_UART_INIT_API(baudrate,bits,stop_bits,parity)                    FK_UART2_INTERRUPT_API(baudrate,bits,stop_bits,parity,modbus_slave_sending,modbus_slave_receiving_and_checking)
//! @brief The API for sending out data
#define MODBUS_SLAVE_SEND_OUT_API(data)						FK_UART2_SEND_BYTE_API(data)
//! @brief The API for receiving data
#define MODBUS_SLAVE_RECEIVE_API()						FK_UART2_GET_DATA_API()
//! @brief The API for keeping MCU don't fall into sleep
#define MODBUS_SLAVE_UART_HIGH_SPEED_API()                                      FK_UART2_HIGH_SPEED_API()
//! @brief The API for cancel MCU don't fall into sleep
#define MODBUS_SLAVE_UART_LOW_SPEED_API()                                       FK_UART2_LOW_SPEED_API()
...
#define MODBUS_SLAVE_START_TX_API()                                             FK_UART2_TX_INTERRUPT_ENABLE_API()
//! @brief THe API for starting UART receive interrupt
#define MODBUS_SLAVE_START_RX_API()                                             FK_UART2_RX_INTERRUPT_ENABLE_API(gb_modbus_slave_buffer)
...
#define MODBUS_SLAVE_UART_TX_DMA_API(buf,size,func)                             FK_UART2_REMAP_TX_DMA_API(PFC_MODBUS_BAUDRATE,PFC_MODBUS_BTIS,PFC_MODBUS_STOP_BITS,PFC_MODBUS_PARITY,buf,size,func)//test
//**********************************************************************************************************************
//
